...
test_or(
    ...
    ex() %>% override_solution("attach(ecgrowth_new);lm(growth ~ tradeshare + yearsschool + rev_coups + rgdp60 + assasinations)") %>% check_function("attach"); ex() %>% override_solution("attach(ecgrowth_new);lm(growth ~ tradeshare + yearsschool + rev_coups + rgdp60 + assasinations)") %>% check_function("lm")
)
```
